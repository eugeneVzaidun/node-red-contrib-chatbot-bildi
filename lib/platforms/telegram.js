...
const { ChatExpress, ChatLog } = require('chat-platform-bildi');
...
const Telegram = new ChatExpress({
  ...
  language: function (payload) {
    return payload != null && payload.from != null
      ? payload.from.language_code
      : null;
  },
  onStop: function () {
    ...
    return new Promise(function (resolve) {
      if (options.connector != null) {
        if (options.connectMode === 'webHook') {
          ...
          if (process.env.PRE_PROD === 'true') {
            resolve();
          } else {
            options.connector.deleteWebHook().then(
              function () {
          ...
          // stop polling
          options.connector.stopPolling().then(
            function () {
              options.connector = null;
              resolve();
            },
  ...
  onStart: function () {
    var options = this.getOptions();
    if (options.connectMode === 'webHook') {
      options.connector = new TelegramBot(options.token, { baseApiUrl: TELEGRAM_API_URL });
      options.connector.sendAnimation = sendAnimation.bind(options.connector);
      return options.connector.getWebHookInfo().then((info) => {
        if (info.url !== options.webHook && info.url !== '') {
          return options.connector.deleteWebHook().then(() => {
      ...
      options.connector = new TelegramBot(options.token, {
        polling: {
          ...
          interval: !isNaN(parseInt(options.polling, 10))
            ? parseInt(options.polling, 10)
            : 1000,
        },
      });
      options.connector.sendAnimation = sendAnimation.bind(options.connector);
  ...
  routes: {
    ...
    '/redbot/telegram': function (req, res) {
      ...
      } else if (json.business_message != null) {
        chatServer.receive(json.business_message);
      }
      res.send({ status: 'ok' });
    },
  },
  ...
  events: {
    ...
    pre_checkout_query: function (botMsg) {
      ...
      // send confirmation
      connector.answerPreCheckoutQuery(botMsg.id, true).then(
        function () {},
        function () {
          chatServer.error(
            'Error on .answerPreCheckoutQuery(), checkout: ' + botMsg.id
          );
        }
      );
    },
...
Telegram.in(function (message) {
  ...
  return new Promise(function (resolve) {
    if (
      _.isArray(botMsg.new_chat_members) &&
      !_.isEmpty(botMsg.new_chat_members)
    ) {
...
Telegram.in(function (message) {
  ...
  return new Promise(function (resolve, reject) {
    ...
    if (message.originalMessage.sticker != null) {
      connector
        .getFileLink(message.originalMessage.sticker.file_id)
...
Telegram.in(function (message) {
  return new Promise(function (resolve) {
    if (
      _.isString(message.originalMessage.text) &&
      !_.isEmpty(message.originalMessage.text)
    ) {
      message.payload.content = message.originalMessage.text;
      message.payload.type = 'message';
      if (helpers.isCommand(message.originalMessage.text)) {
        message.payload.arguments = message.originalMessage.text
          .split(' ')
          .slice(1);
      }
      resolve(message);
...
Telegram.in(function (message) {
  return new Promise(function (resolve) {
    if (
      _.isString(message.originalMessage.business_connection_id) &&
      !_.isEmpty(message.originalMessage.text)
    ) {
      message.payload.content = message.originalMessage.text;
      message.payload.type = 'business_message';
      if (helpers.isCommand(message.originalMessage.text)) {
        message.payload.arguments = message.originalMessage.text
          .split(' ')
          .slice(1);
...
Telegram.in(async function (message) {
  ...
  const { firstName, lastName, username } =
    (await when(context.get('firstName', 'lastName', 'username'))) || {};
  // only change if not defined
  if (
    _.isEmpty(firstName) &&
    !_.isEmpty(message.originalMessage.from.first_name)
  ) {
    vars.firstName = message.originalMessage.from.first_name;
  }
  if (
    _.isEmpty(lastName) &&
    !_.isEmpty(message.originalMessage.from.last_name)
  ) {
    vars.lastName = message.originalMessage.from.last_name;
  }
  if (
    _.isEmpty(username) &&
    !_.isEmpty(message.originalMessage.from.username)
  ) {
  ...
  if (_.isArray(authorizedUsernames) && !_.isEmpty(authorizedUsernames)) {
    if (
      authorizedUsernames.includes(String(userId)) ||
      authorizedUsernames.includes(username)
    ) {
...
Telegram.in(async function (message) {
  ...
  if (type != null) {
    ...
    const retryLimit = Number(
      process.env.FILE_RETRY_LIMIT ? process.env.FILE_RETRY_LIMIT : 4
    );
    let attempts = 0;
    while (attempts <= retryLimit) {
      try {
        ...
        if (attempts >= retryLimit + 1) {
          throw 'Error downloading a file. ' + e;
        }
...
Telegram.out('photo', async function (message) {
  ...
  const result = await connector.sendPhoto(
    message.payload.chatId,
    message.payload.content,
    {
      message_thread_id: getMessageThreadId(message),
      business_connection_id: message.payload.businessConnectionId,
      ...
      reply_to_message_id: param('replyToMessage', false)
...
Telegram.out('document', async function (message) {
  ...
  const result = await connector.sendDocument(
    ...
    },
    {
      ...
      filename: !_.isEmpty(message.payload.filename)
        ? message.payload.filename.replace(/\.[^.]+$/, '')
        : null,
    }
  );
  return setMessageId(message, result.message_id);
...
Telegram.out('reset-buttons', async function (message) {
  ...
  await connector.sendMessage(message.payload.chatId, message.payload.content, {
    message_thread_id: getMessageThreadId(message),
...
Telegram.out('buttons', async function (message) {
  ...
  _(message.payload.buttons).each((button) => {
    ...
    switch (button.type) {
      ...
      default:
        chatServer.warn(
          'Telegram is not able to handle this button type \'' +
            button.type +
            '\''
        );
    }
...
Telegram.out('request', async function (message) {
  ...
  if (message.payload.requestType === 'location') {
    keyboard = [
      [
        {
          text: !_.isEmpty(message.payload.label)
...
Telegram.out('inline-buttons', async function (message) {
  ...
  const context = message.chat();
  const param = params(message);
  const modifyMessageId =
    param('modifyMessageId') || message.originalMessage.modifyMessageId; // for retro compatibility
  ...
  if (modifyMessageId != null) {
    ...
    result = await connector.editMessageReplyMarkup(
      JSON.stringify({
...
Telegram.out(async function (message) {
  ...
  if (deleteMessageId) {
    connector.deleteMessage(message.originalMessage.chatId, deleteMessageId);
    message.payload = {};
  }
  return message;
});
...
Telegram.out('action', async function (message) {
  ...
  await connector.sendChatAction(
    message.payload.chatId,
    message.payload.waitingType != null ? message.payload.waitingType : 'typing',
    {
      message_thread_id: getMessageThreadId(message),
      business_connection_id: message.originalMessage?.business_connection_id,
    }
  );
  return message;
});
...
Telegram.out('inline-query-answer', async function (message) {
  ...
  await connector.answerInlineQuery(
    inlineQueryId,
    message.payload.content,
    inlineOptions
  );
...
Telegram.out('invoice-shipping', async function (message) {
  ...
  const shippingOptions = _(message.payload.shippingOptions).map(function (
    shippingOption
  ) {
    return {
      ...
      prices: [
        {
          label: shippingOption.id,
          amount: Math.floor(shippingOption.amount * 100),
        },
      ],
    };
  });
  const canShip = !_.isEmpty(shippingOptions);
  await connector.answerShippingQuery(
    message.payload.shippingQueryId,
    canShip,
...
Telegram.out(function (message) {
  ...
  if (!_.isEmpty(logfile)) {
    return when(chatContext.all()).then(function (variables) {
...
Telegram.mixin({
  ...
  callbackQuery: function (botMsg) {
    ...
    var answer = null;
    if (
      connector.lastInlineButtons != null &&
      connector.lastInlineButtons[chatId] != null
    ) {
      // find the button with the right value, takes the answer and alert if any
      var button = _(connector.lastInlineButtons[chatId]).findWhere({
        value: botMsg.data,
      });
      if (button != null) {
    ...
    connector
      .answerCallbackQuery(botMsg.id, { text: answer, show_alert: alert })
      .then(function () {
...
Telegram.registerMessageType(
  'action',
  'Action',
  'Send an action message (like typing, ...)'
);
Telegram.registerMessageType(
  'buttons',
  'Buttons',
  'Open keyboard buttons in the client'
);
Telegram.registerMessageType(
  'command',
  'Command',
  'Detect command-like messages'
);
...
Telegram.registerMessageType(
  'inline-query',
  'Inline Query',
  'Receives inline queries'
);
Telegram.registerMessageType('invoice', 'Invoice', 'Send a payment invoice');
Telegram.registerMessageType(
  'invoice-shipping',
  'Invoice Shipping Query',
  'Define an invoice shipping option'
);
Telegram.registerMessageType(
  'location',
  'Location',
  'Send a map location message'
);
...
Telegram.registerMessageType(
  'request',
  'Request',
  'Trigger a request of location or contact'
);
...
Telegram.registerMessageType(
  ...
  (file) => {
    if (
      !_.isEmpty(file.extension) &&
      !videoExtensions.includes(file.extension)
    ) {
      return `Unsupported file format for video node '${
        file.filename
      }', allowed formats: ${videoExtensions.join(', ')}`;
    }
...
Telegram.registerMessageType('video', 'Video', 'Send video message', (file) => {
...
Telegram.registerMessageType(
  ...
  (file) => {
    if (
      !_.isEmpty(file.extension) &&
      !documentExtensions.includes(file.extension)
    ) {
      return `Unsupported file format for document node '${
        file.filename
      }', allowed formats: ${documentExtensions.join(', ')}`;
    }
    return null;
  }
);
Telegram.registerMessageType(
  ...
  (file) => {
    if (
      ...
      return `Unsupported file format for audio node '${
        file.filename
      }', allowed formats: ${audioExtensions.join(', ')}`;
    }
    return null;
  }
);
Telegram.registerMessageType(
  ...
  (file) => {
    if (
      !_.isEmpty(file.extension) &&
      !photoExtensions.includes(file.extension)
    ) {
      return `Unsupported file format for image node '${
        file.filename
      }', allowed formats: ${photoExtensions.join(', ')}`;
    }
  }
);

Telegram.registerParam('replyToMessage', 'boolean', {
  label: 'Reply to message',
  default: false,
});
Telegram.registerParam('hasSpoiler', 'boolean', {
  label: 'Cover the text, video or photo with spolier animation',
  default: false,
});
Telegram.registerParam('protectContent', 'boolean', {
  label: 'Protects the contents of the sent message from forwarding and saving',
  default: false,
});
Telegram.registerParam('silent', 'boolean', {
  label: 'Silent',
  default: false,
  description: 'Deliver the message silently',
});
Telegram.registerParam('modifyMessageId', 'string', {
  label: 'Modify message',
  description: 'The id of the sent message to be modified',
  placeholder: 'Message Id',
  suggestions: ['{{outboundMessageId}}', '{{inboundMessageId}}'],
});
Telegram.registerParam('parseMode', 'select', {
  label: 'Parse Mode',
  default: 'Markdown',
...
Telegram.registerParam('disableWebPagePreview', 'boolean', {
  label: 'Disable web page preview',
  description: 'Disables link previews for links in this message',
});
Telegram.registerParam('deleteMessageId', 'string', {
