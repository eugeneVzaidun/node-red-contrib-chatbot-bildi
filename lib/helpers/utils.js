...
const utils = {
  ...
  isUsedInEnvironment: function(RED, nodeId, environment) {
    ...
    RED.nodes.eachNode(function(currentNode) {
      if (currentNode.type !== 'chatbot-telegram-menu') {
  ...
  isValidMessage: function(msg, node, options) {
  ...
  /**
   * @method getUserId
   * Extract a valid userId from a message
   * @param {Object} msg
   * @return {String}
   */
   getUserId: function(msg) {
    ...
    } else if (_.isObject(msg.payload) && msg.payload.userId != null) {
  ...
  /**
   * @method getMessageThreadId
   * @param {*} msg 
   * @returns {number|null} The message thread ID if found, otherwise null.
   */
  getMessageThreadId: function(msg) {
    const payloadThreadId = msg.payload?.messageThreadId;
    const originalThreadId = msg.originalMessage?.message_thread_id;

    if (typeof payloadThreadId === 'number') {
      return payloadThreadId;
    } else if (typeof originalThreadId === 'number') {
