...
const { ContextProviders, ChatExpress } = require("chat-platform-bildi");
const clc = require("cli-color");
const prettyjson = require("prettyjson");
const path = require("path");

const utils = require("../../lib/helpers/utils");
const mergeNotEmpty = require("../helpers/merge-not-empty");
...
const GetNodeByPath = require("../helpers/get-node-by-path");
...
const GenericBotNode = (
  ...
  return function (n) {
    ...
    let botConfiguration = {
      ...configurator(n, node),
      contextProvider:
        contextStorageNode != null ? contextStorageNode.contextStorage : null,
      contextParams:
        contextStorageNode != null ? contextStorageNode.contextParams : null,
    };

    // check if there's a valid configuration in global settings
    if (platformConfigs[node.botname] != null) {
      var validation = validators.platform[platform](
        platformConfigs[node.botname]
      );
      if (validation != null) {
        ...
        console.log(
          lcd.timestamp() + prettyjson.render(platformConfigs[node.botname])
        );
        console.log(lcd.timestamp() + "");
    ...
    if (botConfiguration.enableMissionControl) {
      ...
      console.log(
        lcd.timestamp() +
          lcd.warn(
            "Using Mission Control context provider (SQLite) for " +
              node.botname +
              "."
          )
      );
      ...
      botConfiguration.contextParams = {
        dbPath:
          RED.settings.RedBot != null
            ? RED.settings.RedBot.dbPath
            : path.join(RED.settings.userDir, "mission-control.sqlite"),
      };
      ...
      console.log(
        lcd.timestamp() +
          lcd.warn(
            "No context provider specified for chatbot " +
              node.botname +
    ...
    if (node.chat == null && validator(botConfiguration)) {
      ...
      node.contextProvider = contextProviders.getProvider(
        botConfiguration.contextProvider,
        // prevents empty chatbotId = '' overwriting a non-empty value
        mergeNotEmpty(botConfiguration.contextParams, {
          id: this.store,
      ...
      try {
        ...
        RED.nodes.eachNode(function (currentNode) {
          if (
            currentNode.type === "chatbot-extend" &&
            !_.isEmpty(currentNode.codeJs) &&
...
const GenericInNode = (platform, RED) => {
  ...
  const isMissionControlEnabled = require("../../lib/helpers/is-mc-enabled")(
    RED
  );
  ...
  return function (config) {
    ...
    const getNodeByPath = GetNodeByPath(RED);
    ...
    this.config = RED.nodes.getNode(
      environment === "production" ? this.botProduction : this.bot
    );
    ...
    if (this.config) {
      ...
      if (node.chat) {
        ...
        nodeGlobalKey =
          platform + "_master_" + this.config.id.replace(".", "_");
        let isMaster = false;
        if (globalContextHelper.get(nodeGlobalKey) == null) {
          ...
          globalContextHelper.set(
            "chatbot_info_" + this.config.id.replace(".", "_"),
            {
              nodeId: node.id,
        ...
        node.chat.on("message", async function (message) {
          ...
          const trackVars = await when(
            context.get(
              `${platform}_currentConversationNode`,
              `${platform}_trackNode`
            )
          );

          // if there's a current converation, then the message must be forwarded
          if (trackVars[`${platform}_currentConversationNode`] != null) {
            // if the current node is master, then relay, if not master do nothing
            if (isMaster) {
              await when(
                context.remove(
                  `${platform}_currentConversationNode`,
                  `${platform}_currentConversationNode_at`
                )
              );
              // relay message to the node if found
              const trackNode = getNodeByPath(
                trackVars[`${platform}_currentConversationNode`]
              );
              if (trackNode && _.isFunction(trackNode.relay)) {
                trackNode.relay(message);
              }
            }
          } else if (trackVars[`${platform}_trackNode`] != null) {
            // if the current node is master, then relay, if not master do nothing
            if (isMaster) {
              await when(
                context.remove(
                  `${platform}_trackNode`,
                  `${platform}_trackNode_at`
                )
              );
              // relay message to the node if found
              const trackNode = getNodeByPath(
                trackVars[`${platform}_trackNode`]
              );
              if (trackNode && _.isFunction(trackNode.relay)) {
                trackNode.relay(message);
              }
        ...
        node.warn(
          `Missing or incomplete configuration in ${_s.capitalize(
            platform
          )} Receiver`
        );
      }
  ...
  return function (config) {
    ...
    this.on("input", async function (message, send, done) {
      ...
      send =
        ...
        function (error) {
          node.error.call(node, error, message);
        };
      // check if valid message or right transport, skip also if it's simulator
      if (
        !isValidMessage(message, node) ||
        getTransport(message) !== platform ||
        isSimulator(message)
      ) {
        done();
        return;
      ...
      if (context != null && node.track && !_.isEmpty(node.wires[0])) {
        await when(
          context.set({
            [`${platform}_currentConversationNode`]: node._path,
            [`${platform}_currentConversationNode_at`]: moment().toISOString(),
          })
        );
      }
      if (context != null && !_.isEmpty(message.payload?.trackNodeId)) {
        await when(
          context.set({
            [`${platform}_trackNode`]: message.payload?.trackNodeId,
            [`${platform}_trackNode_at`]: moment().toISOString(),
          })
        );
      }
      // finally send out
      try {
        ...
        if (node.config.inspectMessages) {
          RED.comms.publish("redbot", {
            topic: "message.out",
            payload: outMessage.sentMessage,
          });
        }
        done();
      } catch (error) {
        ...
        if (node.errorOutput) {
          send(
            node.passThrough || node.track
              ? [null, { ...message, payload: error }]
