...
module.exports = function (RED) {
  ...
  function ChatBotInvoice(config) {
    ...
    this.on("input", (msg) => {
      ...
      if (
        !ChatExpress.isSupported(transport, "message") &&
      ...
      const photoWidth =
        extractValue("integer", "photoWidth", node, msg, false) ||
        extractValue("variable", "photoWidth", node, msg, false);
      const photoHeight =
        extractValue("integer", "photoHeight", node, msg, false) ||
      ...
      const needPhoneNumber = extractValue(
        "boolean",
        "needPhoneNumber",
        node,
        msg,
      ...
      const needShippingAddress = extractValue(
        "boolean",
        "needShippingAddress",
        node,
        msg,
      ...
      template(invoicePayload).then((invoicePayload) => {
        invoicePayload.photoWidth = !_.isEmpty(invoicePayload.photoWidth)
          ? parseInt(invoicePayload.photoWidth, 10)
          : null;
        invoicePayload.photoHeight = !_.isEmpty(invoicePayload.photoHeight)
          ? parseInt(invoicePayload.photoHeight, 10)
          : null;
