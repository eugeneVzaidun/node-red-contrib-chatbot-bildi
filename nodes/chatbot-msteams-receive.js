  ...
  function MicrosoftTeamsBotNode(n) {
    ...
    if (!startNode) {
      ...
      console.log(
        warn(
          "Microsoft Teams Bot " +
            this.botname +
            " will NOT be launched, environment is " +
            environment
        )
      );
      return;
    ...
    console.log(
      green(
        "Microsoft Teams Bot " +
          this.botname +
          " will be launched, environment is " +
          environment
      )
    );
    ...
    var botConfiguration = {
      ...
      appId:
        node.credentials != null && node.credentials.appId != null
          ? node.credentials.appId.trim()
          : null,
      appPassword:
        node.credentials != null && node.credentials.appPassword != null
          ? node.credentials.appPassword.trim()
          : null,
      logfile: node.log,
      contextProvider:
    ...
    if (msteamsConfigs[node.botname] != null) {
      var validation = validators.platform.msteams(
        msteamsConfigs[node.botname]
      );
      if (validation != null) {
        ...
        console.log(
          lcd.error(
            'Found a Microsoft Teams configuration in settings.js "' +
              node.botname +
              "\", but it's invalid."
          )
        );
        ...
        console.log(
          lcd.grey(
            'Found a valid Microsoft Teams configuration in settings.js: "' +
              node.botname +
              '":'
          )
        );
    ...
    if (this.config) {
      ...
      if (node.chat) {
        ...
        node.chat.on("message", function (message) {
          ...
          when(context.get("currentConversationNode")).then(function (
            ...
            if (currentConversationNode != null) {
              ...
              if (isMaster) {
                when(context.remove("currentConversationNode")).then(
                  function () {
        ...
        node.warn(
          "Missing or incomplete configuration in Microsoft Teams Receiver"
        );
      }
    } else {
