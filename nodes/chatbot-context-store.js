...
const { ChatExpress: ChatPlatform } = require("chat-platform-bildi");
...
module.exports = function (RED) {
  ...
  RED.httpNode.get("/redbot/context-providers", function (req, res) {
    ...
    res.send(
      _(contextProviders).map(function (obj, name) {
        return {
          type: name,
          name:
            obj.prototype != null && !_.isEmpty(obj.prototype.name)
              ? obj.prototype.name
              : name,
          description:
            obj.prototype != null && !_.isEmpty(obj.prototype.description)
  ...
  RED.httpNode.get("/redbot/globals", async function (req, res) {
    ...
    ChatPlatform.getPlatforms().forEach(
      (platform) => (result[platform.id] = globalContextHelper.get(platform.id))
    );
    ...
    RED.nodes.eachNode((n) => {
      if (
        n.type === "mc-simulator-receiver" &&
        !result.simulatorChatbotIds.includes(n.chatbotId)
      ) {
    ...
    result.messageTypes = _(ChatPlatform.getMessageTypes()).sortBy(
      (type) => type.label
    );
    // collect events
    result.eventTypes = _(ChatPlatform.getEvents()).sortBy(
      (event) => event.name
    );
    // collect params
